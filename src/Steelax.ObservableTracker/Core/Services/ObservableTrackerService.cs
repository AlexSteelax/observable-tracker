    ...
    where TTrackerHub : class
{
    ...
    public async ValueTask<TResult> WriteAndWaitAsync<TResult>(Func<TTrackerHub, ValueTask<TResult>> factory, CancellationToken cancellationToken = default)
    {
        var cmd = new Command<TResult>(() => factory(_hub));
        await _queueHub.WriteAsync(cmd, cancellationToken);
        var ret = await cmd.WaitAsync();
    ...
    public async ValueTask WriteAndWaitAsync(Func<TTrackerHub, ValueTask> factory, CancellationToken cancellationToken = default)
    {
        var cmd = new Command(() => factory(_hub));
        await _queueHub.WriteAsync(cmd, cancellationToken);
        await cmd.WaitAsync();
    ...
    public ValueTask WriteAsync(Func<TTrackerHub, ValueTask> factory, CancellationToken cancellationToken = default)
    {
        var cmd = new Command(() => factory(_hub));
        return _queueHub.WriteAsync(cmd, cancellationToken);
    }
